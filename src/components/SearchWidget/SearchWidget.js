...
import { ethToIndex } from "../../../lib/ethTools";
import { useETHSearch } from "../../../hooks/use-eth-search";
...
const Wrapper = styled.div`
  ...
  max-width: calc(100vw - ${SCROLLBAR_WIDTH}px - 2rem);

  @media ${querySmallScreen} {
    right: 0.5rem;
    max-width: calc(100vw - ${SCROLLBAR_WIDTH}px - 1rem);
  }
...
function SearchWidget({
  ...
  const { searchETH, currentAddress, nextETH, previousETH } = useETHSearch({
    displayedAddresses,
    virtualPosition,
  });
  const index = React.useMemo(() => {
    if (currentAddress) {
      const index = ethToIndex(currentAddress);
        ...
        >
          <Input
            ...
            onChange={(e) => {
              ...
              searchETH(e.target.value.toLowerCase());
            }}
          />
        </Form>
        <Line />
        <Button onClick={() => previousETH()}>
          <ChevronUp style={{ height: "100%", width: "100%" }} />
        </Button>
        <Button onClick={() => nextETH()}>
